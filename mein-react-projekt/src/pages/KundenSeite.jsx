...
export default function KundenSeite({ benutzer, onLogout }) {
  ...
  const [aktiverTab, setAktiverTab] = useState("termine");
  ...
  const ladeNachrichten = useCallback(async () => {
    const res = await fetch(`${API}/nachrichten/${benutzer.id}`);
    const data = await res.json();
    setNachrichten(Array.isArray(data) ? data : []);
  }, [benutzer.id]);

  const ladeUngelesen = useCallback(async () => {
    const res = await fetch(`${API}/nachrichten/ungelesen/${benutzer.id}`);
    const data = await res.json();
    setNachrichtenBadge(data.count || 0);
  }, [benutzer.id]);
  ...
  useWebSocket(() => {
    ...
    ladeAlles();
    ladeUngelesen();
    if (aktiverTab === "nachrichten") ladeNachrichten();
  ...
  // Nachrichten laden wenn Tab geöffnet wird
  useEffect(() => {
    if (aktiverTab === "nachrichten") {
      ladeNachrichten();
      setNachrichtenBadge(0); // Optimistisch zurücksetzen
    }
  }, [aktiverTab, ladeNachrichten]);
  ...
  async function nachrichtSenden() {
    ...
    const res = await fetch(`${API}/nachrichten`, {
      ...
      body: JSON.stringify({ sender_id: benutzer.id, text: nachrichtText.trim() }),
    });
  ...
  function fmtKurz(dt) {
    const d = new Date(dt);
    return { zeit: d.toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" }), d };
  }
  ...
  const tabs = [
    { id: "termine", label: "Termine", badge: null },
      ...
      <div style={{ ...cs.container, opacity: mounted ? 1 : 0, transform: mounted ? "none" : "translateY(16px)", transition: "all 0.5s ease" }}>
        ...
        <div style={cs.tabBar}>
          {tabs.map(({ id, label, badge }) => (
            <button
              key={id}
              style={{ ...cs.tabBtn, ...(aktiverTab === id ? cs.tabBtnActive : {}) }}
              onClick={() => setAktiverTab(id)}
            >
              {label}
              {badge && <span style={cs.tabBadge}>{badge}</span>}
            </button>
          ))}
        </div>
            ...
            {meineAnfrage && !meinAktiverTermin && (
              <div style={cs.statusCard}>
                <div style={cs.statusCardInner}>
                  <div style={cs.statusBadge}>⏳ Ausstehend</div>
                      ...
                      >
                        ...
                        {tT.length > 0 && !istVergangen && (
                          <div style={{ display: "flex", gap: "2px", marginTop: "4px", justifyContent: "center" }}>
                            {istMeinerGebucht && <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: istGewählt ? "white" : "#22c55e" }} />}
                            {istMeinerAngefragt && <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: istGewählt ? "white" : "#f59e0b" }} />}
                            {istAbgesagt && <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: istGewählt ? "rgba(255,255,255,0.7)" : "#ef4444" }} />}
                            {offenCount > 0 && <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: istGewählt ? "rgba(255,255,255,0.6)" : "#c8b49a" }} />}
                          </div>
                        )}
                      </div>
                    );
                      ...
                      {tagTermine.map(t => {
                        ...
                        return (
                          <div key={t.termin_id} style={{ ...cs.slotRow, background: slotBg, borderColor: slotBorder }}>
                            ...
                            <div style={{ flex: 1 }}>
                              {meinerStatus === "gebucht" && <span style={cs.slotLabel}>✓ Bestätigt</span>}
                              {meinerStatus === "angefragt" && <span style={{ ...cs.slotLabel, background: "rgba(245,158,11,0.12)", color: "#92400e" }}>⏳ Ausstehend</span>}
                              {meinerStatus === "abgesagt" && (
                                <div>
                                  <span style={{ ...cs.slotLabel, background: "rgba(251,146,60,0.15)", color: "#c2410c" }}>✕ Vom Friseur abgesagt</span>
                                  {t.absage_grund && <div style={{ fontSize: "11px", color: "#c2410c", marginTop: "4px", fontStyle: "italic" }}>Grund: „{t.absage_grund}"</div>}
                                </div>
                              )}
                              {meinerStatus === "abgelehnt" && (
                                <div>
                                  <span style={{ ...cs.slotLabel, background: "rgba(239,68,68,0.1)", color: "#b91c1c" }}>✕ Abgelehnt</span>
                                  {t.absage_grund && <div style={{ fontSize: "11px", color: "#b91c1c", marginTop: "4px", fontStyle: "italic" }}>Grund: „{t.absage_grund}"</div>}
                                </div>
                              )}
                              {istOffen && !hatAktivenTermin && <span style={cs.slotFreeLabel}>Freier Termin – jetzt anfragen</span>}
                              {istOffen && hatAktivenTermin && <span style={{ fontSize: "13px", color: "#c8b49a" }}>Freier Termin</span>}
                              {fremdGebucht && (
                                <div>
                                  <span style={{ fontSize: "13px", color: "#c8b49a" }}>Belegt</span>
                                  {!hatAktivenTermin && <div style={{ fontSize: "11px", color: "#c8b49a", marginTop: "2px" }}>{drauf ? "Auf Warteliste" : "Warteliste verfügbar"}</div>}
                                </div>
                              )}
                            ...
                            <div>
                              {meinerStatus === "gebucht" ? (
                                ...
                                <button style={cs.slotPrimaryBtn} onClick={() => anfragen(t.termin_id)}>Anfragen →</button>
                              ) : fremdGebucht && !hatAktivenTermin ? (
                                <button style={drauf ? cs.slotWaitlistActive : cs.slotWaitlistBtn} onClick={() => warteToggle(t.termin_id, drauf)}>
                                  {drauf ? "✓ Liste" : "+ Liste"}
                                </button>
                              ) : istOffen && hatAktivenTermin ? (
        ...
        {aktiverTab === "nachrichten" && (
          <div style={{ ...cs.card, display: "flex", flexDirection: "column", height: "520px" }}>
            {/* Chat-Header */}
            <div style={cs.chatHeader}>
              <div style={cs.chatAvatar}>✂</div>
              <div>
                <p style={cs.chatName}>Friseur – Atelier</p>
                <p style={cs.chatSub}>Nur für dich sichtbar</p>
              </div>
            </div>
            ...
            <div style={cs.chatBody}>
              {nachrichten.length === 0 ? (
                <div style={{ textAlign: "center", margin: "auto" }}>
                  <p style={{ color: "#d4c5b5", fontSize: "13px", marginBottom: "6px" }}>Noch keine Nachrichten</p>
                  <p style={{ color: "#e8ddd5", fontSize: "12px" }}>Schreib dem Friseur direkt hier.</p>
                </div>
              ) : (
                nachrichten.map(n => {
                  const vonAdmin = n.sender_typ === "admin";
                  return (
                    <div key={n.nachrichten_id} style={{ display: "flex", justifyContent: vonAdmin ? "flex-start" : "flex-end", marginBottom: "8px" }}>
                      <div style={{
                        ...
                        color: vonAdmin ? "#2c1810" : "#e8d5be",
                        borderColor: vonAdmin ? "#ede8e0" : "transparent",
                        borderRadius: vonAdmin ? "16px 16px 16px 4px" : "16px 16px 4px 16px",
                        boxShadow: vonAdmin ? "0 1px 4px rgba(92,61,46,0.08)" : "0 2px 8px rgba(44,24,16,0.2)",
                      }}>
                        {vonAdmin && <p style={{ margin: "0 0 4px", fontSize: "10px", color: "#a0887a", fontWeight: "600", textTransform: "uppercase", letterSpacing: "0.06em" }}>Friseur</p>}
                        <p style={{ margin: "0 0 4px", fontSize: "13px", lineHeight: 1.5 }}>{n.text}</p>
                        <p style={{ margin: 0, fontSize: "10px", opacity: 0.55, textAlign: "right" }}>{fmtZeit(n.erstellt_am)}</p>
                      </div>
                    </div>
                  );
                })
            ...
            <div style={cs.chatInput}>
              <input
                ...
                placeholder="Nachricht an den Friseur …"
                value={nachrichtText}
                onChange={e => setNachrichtText(e.target.value)}
                onKeyDown={e => e.key === "Enter" && nachrichtSenden()}
              />
              <button
                style={{ ...cs.sendBtn, opacity: nachrichtText.trim() ? 1 : 0.4 }}
...
const cs = {
  page: { minHeight: "100vh", background: "#f7f4f0", fontFamily: "'DM Sans', sans-serif", padding: "0 0 3rem", position: "relative", overflowX: "hidden" },
  bgOrb1: { position: "fixed", top: "0", left: "0", right: "0", height: "200px", background: "linear-gradient(to bottom, rgba(92,61,46,0.06) 0%, transparent 100%)", pointerEvents: "none" },
  bgOrb2: { position: "fixed", bottom: "0", right: "0", width: "300px", height: "300px", borderRadius: "50%", background: "radial-gradient(circle, rgba(180,155,120,0.08) 0%, transparent 70%)", pointerEvents: "none" },
  container: { maxWidth: "700px", margin: "0 auto", padding: "0 16px", display: "flex", flexDirection: "column", gap: "14px" },
  header: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 0 8px" },
  headerLeft: { display: "flex", alignItems: "center", gap: "12px" },
  logo: { width: "40px", height: "40px", background: "linear-gradient(135deg, #2c1810 0%, #5c3d2e 100%)", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px", color: "#e8d5be", boxShadow: "0 4px 12px rgba(44,24,16,0.2)" },
  salonName: { margin: "0 0 2px", fontSize: "18px", fontWeight: "600", fontFamily: "'Cormorant Garamond', serif", color: "#2c1810", letterSpacing: "0.04em" },
  ...
  logoutBtn: { padding: "7px 14px", border: "1px solid #e8e0d8", borderRadius: "8px", background: "white", cursor: "pointer", fontSize: "12px", color: "#8b6347", fontFamily: "'DM Sans', sans-serif" },
  toast: { padding: "10px 14px", borderRadius: "10px", fontSize: "13px", border: "1px solid", fontWeight: "500", animation: "slideDown 0.25s ease" },
  // Tabs
  tabBar: { display: "flex", gap: "2px", borderBottom: "2px solid #ede8e0" },
  tabBtn: { padding: "9px 16px", border: "none", background: "transparent", cursor: "pointer", fontSize: "13px", color: "#a0887a", borderBottom: "2px solid transparent", marginBottom: "-2px", borderRadius: "4px 4px 0 0", fontFamily: "'DM Sans', sans-serif", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: "6px", transition: "color 0.15s" },
  tabBtnActive: { color: "#2c1810", fontWeight: "600", borderBottom: "2px solid #2c1810" },
  tabBadge: { background: "#2c1810", color: "#e8d5be", borderRadius: "10px", fontSize: "10px", fontWeight: "700", padding: "1px 6px", minWidth: "18px", textAlign: "center" },
  // Benachrichtigungen
  cancelledBanner: { display: "flex", flexDirection: "column", gap: "14px", padding: "18px", background: "linear-gradient(135deg, rgba(245,158,11,0.08) 0%, rgba(255,255,255,0.95) 100%)", border: "1.5px solid rgba(245,158,11,0.4)", borderRadius: "16px", animation: "slideDown 0.3s ease", boxShadow: "0 4px 16px rgba(245,158,11,0.08)" },
  cancelledBannerLeft: { display: "flex", gap: "14px", alignItems: "flex-start" },
  cancelledIcon: { width: "36px", height: "36px", borderRadius: "50%", flexShrink: 0, background: "rgba(245,158,11,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", fontWeight: "700", color: "#d97706" },
  cancelledTitle: { margin: "0 0 4px", fontWeight: "600", fontSize: "14px", color: "#92400e" },
  cancelledDate: { margin: "0 0 8px", fontSize: "13px", color: "#78350f", fontFamily: "'Cormorant Garamond', serif", fontWeight: "500" },
  cancelledReasonBox: { display: "inline-flex", gap: "6px", alignItems: "baseline", padding: "8px 14px", background: "rgba(255,255,255,0.8)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: "10px", fontSize: "13px" },
  ...
  cancelledAction: { display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: "12px", borderTop: "1px solid rgba(245,158,11,0.2)", flexWrap: "wrap", gap: "8px" },
  cancelledHint: { margin: 0, fontSize: "12px", color: "#c8a97a" },
  cancelledBtn: { padding: "8px 18px", background: "linear-gradient(135deg, #2c1810 0%, #5c3d2e 100%)", color: "#e8d5be", border: "none", borderRadius: "10px", cursor: "pointer", fontSize: "12px", fontWeight: "500", fontFamily: "'DM Sans', sans-serif", boxShadow: "0 2px 8px rgba(44,24,16,0.2)" },
  // Status
  statusCard: { background: "white", border: "1.5px solid #ede8e0", borderRadius: "16px", overflow: "hidden", boxShadow: "0 2px 16px rgba(92,61,46,0.06)" },
  statusCardInner: { padding: "18px 18px 14px" },
  statusBadge: { display: "inline-flex", alignItems: "center", padding: "4px 12px", borderRadius: "20px", fontSize: "11px", fontWeight: "600", background: "rgba(245,158,11,0.12)", color: "#92400e", marginBottom: "10px", letterSpacing: "0.02em" },
  ...
  noteBox: { marginTop: "10px", padding: "10px 12px", background: "#f0f7ff", border: "1px solid #bae6fd", borderRadius: "10px", fontSize: "13px", color: "#0369a1" },
  statusActions: { padding: "12px 18px", borderTop: "1px solid #f0ebe4", background: "#fdfcfb" },
  ghostBtn: { padding: "8px 16px", border: "1px solid #e8e0d8", borderRadius: "8px", background: "white", cursor: "pointer", fontSize: "12px", color: "#8b6347", fontFamily: "'DM Sans', sans-serif" },
  ghostBtnDanger: { padding: "8px 16px", border: "1px solid #fca5a5", borderRadius: "8px", background: "white", cursor: "pointer", fontSize: "12px", color: "#b91c1c", fontFamily: "'DM Sans', sans-serif" },
  tooLateHint: { margin: 0, fontSize: "12px", color: "#c8b49a" },
  emptyCard: { display: "flex", alignItems: "center", gap: "12px", padding: "18px", background: "white", border: "1px solid #ede8e0", borderRadius: "14px" },
  emptyText: { margin: 0, fontSize: "13px", color: "#c0a898" },
  infoCard: { display: "flex", gap: "10px", alignItems: "flex-start", padding: "12px 16px", background: "rgba(59,130,246,0.05)", border: "1px solid rgba(59,130,246,0.15)", borderRadius: "12px", fontSize: "13px" },
  section: { display: "flex", flexDirection: "column", gap: "0" },
  sectionTitle: { margin: "0 0 10px", fontSize: "13px", fontWeight: "500", color: "#a0887a", textTransform: "uppercase", letterSpacing: "0.1em" },
  card: { background: "white", border: "1px solid #ede8e0", borderRadius: "16px", padding: "18px", boxShadow: "0 2px 12px rgba(92,61,46,0.05)" },
  ...
  navBtn: { width: "32px", height: "32px", border: "1px solid #ede8e0", borderRadius: "8px", background: "white", cursor: "pointer", fontSize: "14px", color: "#8b6347", display: "flex", alignItems: "center", justifyContent: "center" },
  calGrid: { display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: "3px" },
  calHeaderCell: { textAlign: "center", fontSize: "10px", fontWeight: "600", color: "#c0a898", textTransform: "uppercase", letterSpacing: "0.06em", padding: "4px 0 10px" },
  calDay: { borderRadius: "8px", padding: "8px 4px 6px", display: "flex", flexDirection: "column", alignItems: "center", border: "1px solid transparent", background: "transparent", minHeight: "50px", transition: "all 0.15s" },
  calDayToday: { border: "1.5px solid #8b6347", background: "rgba(139,99,71,0.04)" },
  calDaySelected: { background: "linear-gradient(135deg, #2c1810 0%, #5c3d2e 100%)", border: "1.5px solid #2c1810", boxShadow: "0 2px 8px rgba(44,24,16,0.2)" },
  calDayHasSlots: { background: "rgba(139,99,71,0.04)", border: "1px solid rgba(139,99,71,0.2)" },
  ...
  slotRow: { display: "flex", alignItems: "center", gap: "12px", padding: "12px 14px", borderRadius: "10px", border: "1.5px solid #ede8e0", transition: "all 0.15s" },
  slotTime: { fontSize: "16px", fontWeight: "700", minWidth: "48px", fontFamily: "'Cormorant Garamond', serif" },
  slotLabel: { display: "inline-block", padding: "2px 10px", borderRadius: "20px", fontSize: "11px", fontWeight: "600", background: "rgba(74,222,128,0.12)", color: "#166534" },
  slotFreeLabel: { fontSize: "13px", color: "#5c3d2e", fontWeight: "500" },
  slotNote: { fontSize: "11px", color: "#0369a1", marginTop: "4px" },
  slotPrimaryBtn: { padding: "8px 18px", background: "linear-gradient(135deg, #2c1810 0%, #5c3d2e 100%)", color: "#e8d5be", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "13px", fontWeight: "600", fontFamily: "'DM Sans', sans-serif", whiteSpace: "nowrap", boxShadow: "0 2px 8px rgba(44,24,16,0.25)", letterSpacing: "0.02em" },
  slotGhostBtn: { padding: "6px 12px", border: "1px solid #e8d8c0", borderRadius: "8px", background: "white", cursor: "pointer", fontSize: "11px", color: "#8b6347", fontFamily: "'DM Sans', sans-serif", whiteSpace: "nowrap" },
  slotDangerBtn: { padding: "6px 12px", border: "1px solid #fca5a5", borderRadius: "8px", background: "white", cursor: "pointer", fontSize: "11px", color: "#b91c1c", fontFamily: "'DM Sans', sans-serif", whiteSpace: "nowrap" },
  slotWaitlistBtn: { padding: "6px 12px", border: "1px solid #e8e0d8", borderRadius: "8px", background: "white", cursor: "pointer", fontSize: "11px", color: "#a0887a", fontFamily: "'DM Sans', sans-serif", whiteSpace: "nowrap" },
  slotWaitlistActive: { padding: "6px 12px", border: "1px solid rgba(59,130,246,0.3)", borderRadius: "8px", background: "rgba(59,130,246,0.06)", cursor: "pointer", fontSize: "11px", color: "#1d4ed8", fontWeight: "500", fontFamily: "'DM Sans', sans-serif", whiteSpace: "nowrap" },
  // Chat
  chatHeader: { display: "flex", alignItems: "center", gap: "10px", paddingBottom: "14px", borderBottom: "1px solid #f0ebe4", flexShrink: 0 },
  chatAvatar: { width: "36px", height: "36px", flexShrink: 0, borderRadius: "50%", background: "linear-gradient(135deg, #2c1810 0%, #5c3d2e 100%)", color: "#e8d5be", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px" },
  chatName: { margin: 0, fontSize: "13px", fontWeight: "600", color: "#2c1810" },
  chatSub: { margin: 0, fontSize: "10px", color: "#c0a898" },
  chatBody: { flex: 1, overflowY: "auto", padding: "14px 0", display: "flex", flexDirection: "column" },
  chatInput: { display: "flex", gap: "8px", alignItems: "center", paddingTop: "12px", borderTop: "1px solid #f0ebe4", flexShrink: 0 },
  chatTextInput: { flex: 1, padding: "10px 14px", border: "1.5px solid #ede8e0", borderRadius: "10px", fontSize: "13px", fontFamily: "'DM Sans', sans-serif", color: "#2c1810", background: "#fdfcfb", outline: "none" },
  sendBtn: { width: "40px", height: "40px", flexShrink: 0, background: "linear-gradient(135deg, #2c1810 0%, #5c3d2e 100%)", color: "#e8d5be", border: "none", borderRadius: "10px", cursor: "pointer", fontSize: "14px", display: "flex", alignItems: "center", justifyContent: "center" },
};